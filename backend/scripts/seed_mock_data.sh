...
declare -a AGENT_DESCRIPTIONS=(
    "Observes runtime state, service health, and unexpected anomalies in real time."
    "Analyzes incidents, traces root causes, and provides diagnosis summaries."
    "Applies risk controls, approval gates, and operation safety checks."
    "Maintains storage health, telemetry snapshots, and recovery workflows."
)
declare -a AGENT_TOKENS=(
    "mock-observer-token"
    "mock-diagnoser-token"
    "mock-guardian-token"
...
api_post "/messages" '{"agent_id":"Guardian","level":"ERROR","content":"Approval gate blocked one high-risk operation pending manual review."}' > /dev/null
api_post "/messages" '{"agent_id":"Diagnoser","level":"ERROR","content":"Root-cause correlation stalled due to missing upstream trace segment."}' > /dev/null
...
api_post "/messages" '{"agent_id":"Observer","level":"INFO","content":"Runtime observability stream is healthy and ingest latency is within target."}' > /dev/null
api_post "/messages" '{"agent_id":"Observer","level":"WARNING","content":"A transient spike was detected on request latency and auto-recovered."}' > /dev/null
...
OBSERVER_ID=$(printf '%s' "$AGENTS_JSON" | find_agent_id "Observer")
DIAGNOSER_ID=$(printf '%s' "$AGENTS_JSON" | find_agent_id "Diagnoser")
GUARDIAN_ID=$(printf '%s' "$AGENTS_JSON" | find_agent_id "Guardian")
...
OBSERVER_THREAD=$(create_thread "$OBSERVER_ID" "agent_direct" "Runtime Observation" "Observe service state, execution flow health, and anomaly signals." "true")
seed_thread_chat "$OBSERVER_THREAD" "$ADMIN_USER" "Observer" "先给我一个当前运行时状态和异常概览。"
seed_thread_chat "$OBSERVER_THREAD" "Observer" "$ADMIN_USER" "整体运行时可用，刚出现过一次短暂延迟抖动，目前已恢复并持续监控中。"

DIAGNOSER_THREAD=$(create_thread "$DIAGNOSER_ID" "agent_direct" "Root Cause Desk" "Diagnose incidents with chain-of-causation summaries and next checks." "false")
seed_thread_chat "$DIAGNOSER_THREAD" "$ADMIN_USER" "Diagnoser" "昨天那次执行超时，你先做下归因分析。"
seed_thread_chat "$DIAGNOSER_THREAD" "Diagnoser" "$ADMIN_USER" "初步归因为上游依赖抖动叠加连接池水位不足，建议先扩容并补一轮压测验证。"

GUARDIAN_THREAD=$(create_thread "$GUARDIAN_ID" "agent_direct" "Risk Gate" "Control risk with approval gates before high-impact operations." "false")
seed_thread_chat "$GUARDIAN_THREAD" "$ADMIN_USER" "Guardian" "有高风险运维动作时，先走你的审批门禁。"
seed_thread_chat "$GUARDIAN_THREAD" "Guardian" "$ADMIN_USER" "已开启高风险门禁策略：未通过审批的破坏性操作将被自动阻断。"
